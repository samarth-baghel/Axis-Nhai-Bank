...
export class GenerateReportsComponent implements OnInit {
  ...
  onROBOByDayClk(fromDate?: any,toDate?: any) {

    let isValidFromDate = this.dateValidation(fromDate);
    let isValidToDate = this.dateValidation(toDate)
    ...
    let toFinal = '';
    if (fromDate && toDate) {
      let dateFrom = new Date(fromDate.getFullYear(), fromDate.getMonth(), fromDate.getDate());
      let date1 = new Date();
      date1.setHours(0, 0, 0, 0);
      if (dateFrom.getTime() == date1.getTime()) {
        dateFrom = new Date(date1.getFullYear(), date1.getMonth(), date1.getDate() - 1);
      }
      finalVal = this.baseService.dateFormatDDMMYYY(dateFrom);
      let datetodate = new Date(toDate.getFullYear(), toDate.getMonth(), toDate.getDate());
      let date2 = new Date();
